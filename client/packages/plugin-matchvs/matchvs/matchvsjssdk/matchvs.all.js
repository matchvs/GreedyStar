...
var LocalConf = {
    ...
    GETHOSTLIST_URL :   "http://sdk.matchvs.com",          //hostlist请求地址
    ...
    this.handleMsg = function (msg) {
        ...
        switch (header.cmd) {
            ...
            case MATCHVS_USER_LOGIN_RSP:
                ...
                packet.payload = proto.stream.LoginRsp.deserializeBinary(
                    msg.buffer.slice(FIXED_HEAD_SIZE,msg.buffer.byteLength));
                break;
            case MATCHVS_ROOM_JOIN_RSP:
                // packet.payload = mPortoBufRootGW.lookup("stream.JoinRoomRsp").decode(ext);
                packet.payload = proto.stream.JoinRoomRsp.deserializeBinary(
                    msg.buffer.slice(FIXED_HEAD_SIZE,msg.buffer.byteLength));

                break;
            case MATCHVS_ROOM_CREATE_RSP:
                packet.payload = proto.stream.CreateRoomRsp.deserializeBinary(
                    msg.buffer.slice(FIXED_HEAD_SIZE,msg.buffer.byteLength));
                break;
...
function MatchvsEngine() {
     ...
     */
    this.login = function (pUserID, pToken, pGameID, pGameVersion, pAppKey,
                           pSecretKey, pDeviceID, pGatewayID) {
        ...
        var buf = this.mProtocol.login(pUserID, pToken, pGameID,
            pGameVersion, pAppKey, pSecretKey,
            pDeviceID, pGatewayID);
    ...
    this.createRoom = function (createRoomInfo, userProfile) {
        if((this.mEngineState & ENGIN_ESTATE.STATE_HAVE_LOGIN) !== ENGIN_ESTATE.STATE_HAVE_LOGIN) return -4;//未登录
        //正在加入房间不允许创建房间
        if((this.mRoomLock & ENGIN_ESTATE.STATE_JOIN_ROOMING) === ENGIN_ESTATE.STATE_JOIN_ROOMING) return -7;
        //设置用户正在创建房间
        ...
        this.mNetWork.send(this.mProtocol.roomCreate(createRoomInfo.maxPlayer, 0,
            this.mGameID, roomInfo, playInfo));
        return 0;
    };
    ...
    this.getRoomList = function (filter) {
        if((this.mEngineState & ENGIN_ESTATE.STATE_HAVE_INIT) !== ENGIN_ESTATE.STATE_HAVE_INIT)return -2;//初始化
        if((this.mEngineState & ENGIN_ESTATE.STATE_HAVE_LOGIN) !== ENGIN_ESTATE.STATE_HAVE_LOGIN)return -4;//未登录
        if((this.mEngineState & ENGIN_ESTATE.STATE_JOIN_ROOMING) === ENGIN_ESTATE.STATE_JOIN_ROOMING)return -7;//加入房间中
    ...
    this.joinRandomRoom = function (maxPlayer, userProfile) {
        if((this.mEngineState & ENGIN_ESTATE.STATE_HAVE_LOGIN) !== ENGIN_ESTATE.STATE_HAVE_LOGIN)return -4;//未登录
        //正在加入房间设置用户正在
        if((this.mRoomLock & ENGIN_ESTATE.STATE_CREATEROOM) === ENGIN_ESTATE.STATE_CREATEROOM)return -1;

        if(maxPlayer > MVSCONFIG.MAXPLAYER_LIMIT)return -20;
    ...
    this.joinRoom = function (roomID,userProfile) {
        ...
        if (0 === roomId||roomId === " " || roomId === "") return 1;
...
module.exports = {MatchvsEngine, MatchvsResponse, MatchInfo: MsMatchInfo, CreateRoomInfo: MsCreateRoomInfo, RoomFilter: MsRoomFilter, LocalStore_Clear};
