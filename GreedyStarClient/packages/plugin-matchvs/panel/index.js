...
var adm_zip = Editor.require('packages://plugin-matchvs/node_modules/adm-zip/adm-zip.js')
var MtaH5 = Editor.require('packages://plugin-matchvs/node_modules/mta-h5-analysis/index.js');
var readTime;
        ...
        window.plugin = new window.Vue({
            ...
            methods: {
                        ...
                        if (request.status === 200) {
                            ...
                            for (var i = 0; i < data.length; i++) {
                                if ("JavaScript" === data[i].title) {
                                    ...
                                    self.versionsNum = data[i].versions.substring(12);
                ...
                onCheckConfiguration() {
                    MtaH5.clickStat("checkconfiguration");
                    let projectPath = Editor.projectInfo.path;
                    var self = this;
                ...
                unZipPlugin(filename, rsp) {
                    ...
                    fsorigin.writeFile(filePath, int8View, function (err) {
                        if (err) {
                            ...
                            unzip.extractAllTo( PATH.join(projectPath, "packages/"), true);
                ...
                readJsSDKDir() {
                    var self = this;
                    let projectPath = Editor.projectInfo.path;
                    let JS_SDK_PATH = PATH.join(projectPath, "packages/plugin-matchvs/matchvs/matchvsjssdk/")
                    fsorigin.readdir(PATH.join(projectPath, "packages/"), function (err, files) {
                        if (err) {
                            MtaH5.clickStat("updatefailed");
                            return false;
                        }
                        files.forEach(function (filename) {
                            if (filename == "SDK_REL_V"+self.versionsNum) {
                                FS.readFile(PATH.join(projectPath,  "packages/SDK_REL_V"+self.versionsNum+"/matchvs.all.js"), 'utf-8', function (err, bytesRead) {
                                    if (err) {
                                        throw err;
                                        self._addLog('解压失败，从从官网手动下载包体进行安装');
                                        return;
                                    }
									self.showAlert.progress = 80;
                                    
                                    fsorigin.writeFileSync(PATH.join(JS_SDK_PATH, 'matchvs.all.js'),bytesRead);
									clearInterval(readTime);
                                });
                                self.deleteFile(PATH.join(projectPath, "packages/JsSdk.zip"));
                                self.showAlert.progress = 90;
                                self.deleteFile(PATH.join(projectPath, "packages/SDK_REL_V"+self.versionsNum+"/matchvs.all.js"));
								self.deleteFile(PATH.join(projectPath, "packages/SDK_REL_V"+self.versionsNum));
                                self.showAlert.progress = 100;
                                self.showAlert.type = "3";
                                self.writeTime();
                                self._addLog("SDK更新完成，请不要将Matchvs.all.js文件设置为插件文件");
                                MtaH5.clickStat("updatesuccess");
                            }
                        });
                    });
                },
